...
with open("files/trump_2020.txt", "r") as file:
    trump_2_text = file.read()

doc_trump_2 = nlp(trump_2_text)

print(f"Polarity: {doc_trump_2._.blob.polarity:.2f}")

# 2
trump_2_sentence_polarity = []

for i, sentence in enumerate(doc_trump_2.sents):
    polarity = sentence._.blob.polarity
    trump_2_sentence_polarity.append({
        "n" : i + 1,
        "trump_polarity" : polarity
    })
df_trump_2 = pd.DataFrame(trump_2_sentence_polarity)

chart3 = alt.Chart(df_trump_2).mark_line().encode(
    alt.X("n", title = "Sentence Number"),
    alt.Y("trump_polarity", title = "Polarity")
).properties(
    title = "Trump 2020",
    width = 100,
    height = 100
